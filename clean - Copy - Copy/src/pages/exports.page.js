(function(){
  ...
  const quantity = round2(row.querySelector('.quantity').value);
    ...
    const existingCustomer = GM_state.customers.find(c => c.phone === customerPhone);
    if (existingCustomer) {
      if (existingCustomer.name !== customerName || existingCustomer.address !== customerAddress || existingCustomer.type !== customerType) {
        const newCustomer = {
          id: GM_utils.uid(),
          name: customerName,
          phone: customerPhone,
        ...
        GM_state.customers.push(newCustomer);
      }
    } else {
      const newCustomer = {
        id: GM_utils.uid(),
    ...
    GM_receipts.createExport(exportData).then(async (result) => {
      await GM_stateAPI.persistAll();
